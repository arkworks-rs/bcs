...
// FieldElementSize}; use ark_std::{marker::PhantomData, test_rng, One};
...
// VerifierMessage},         oracles::RoundOracle,
//         prover::IOPProver,
...
// use crate::{
//     simple_sumcheck::{
//         SimpleSumcheck, SumcheckOracleRef, SumcheckProverParameter,
// SumcheckPublicInput,         SumcheckVerifierParameter,
//     },
//     test_utils::{poseidon_parameters, FieldMTConfig},
// };

// /// This protocol takes 2 polynomial coefficients as private input (as well
// as /// its sum over summation domain). The protocol send those three oracles
// to /// verifier, and run simple sumcheck on each of them.
...
//     sums: (F, F), // sum of `poly0` over summation domain, sum of `poly1`
// over summation domain }
...
//         transcript.send_univariate_polynomial(prover_parameter.degrees.0,
// &poly0)?;         let poly0_ref =
// transcript.submit_prover_current_round(namespace, iop_trace!("poly0"))?;
//         // invoke sumcheck polynomial on first polynomial
//         let ns0 = transcript.new_namespace(namespace, iop_trace!("first
// sumcheck protocol"));         SimpleSumcheck::prove(
//             ns0,
...
//         transcript.send_univariate_polynomial(prover_parameter.degrees.1,
// &poly1)?;         let poly1_ref =
// transcript.submit_prover_current_round(namespace, iop_trace!("poly1"))?;

//         // invoke sumcheck polynomial on second polynomial
//         let ns1 = transcript.new_namespace(namespace, iop_trace!("second
// sumcheck protocol"));         SimpleSumcheck::prove(
//             ns1,
...
//     {
//         // verifier send r0
//         transcript.squeeze_verifier_field_elements(&[FieldElementSize::
// Full]);         transcript.submit_verifier_current_round(namespace,
// iop_trace!("r0"));
...
//         let asserted_sums = (public_input.sums.0 * r0, public_input.sums.1 *
// r1);
...
// /// Multiround version of a simple univariate sumcheck (currently without
// ldt, /// which is completely insecure). We assume that size of summation
// domain < /// degree of testing poly < size of evaluation domain
// #[test]
...
//         num_queries: 3,
//     };
//     let claimed_sum1 = Radix2CosetDomain::new(summation_domain.clone(),
// Fr::one())         .evaluate(&poly0)
//         .into_iter()
//         .sum::<Fr>();
//     let claimed_sum2 = Radix2CosetDomain::new(summation_domain.clone(),
// Fr::one())         .evaluate(&poly1)
//         .into_iter()
//         .sum::<Fr>();
...
//     let result = BCSVerifier::verify::<SumcheckMultiroundExample<Fr>,
// LinearCombinationLDT<Fr>, _>(         sponge,
//         &proof,
//         &vp,
//         &verifier_param,
//         &ldt_parameter,
