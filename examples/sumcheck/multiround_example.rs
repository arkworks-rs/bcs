...
use ark_bcs::{
    ...
    iop::{
        bookkeeper::{BookkeeperContainer, NameSpace, ToMsgRoundRef},
    ...
    for SumcheckMultiroundExample<F>
{
    ...
    fn query_and_decide<O: RoundOracle<F>>(
        ...
        let poly1_ref =
            SumcheckOracleRef::new((namespace, 1).to_prover_msg_round_ref(transcript_messages));

        // get the r0, r1 we squeezed in commit phase
        let r0 = transcript_messages.verifier_round((namespace, 0))[0]
            .clone()
            .try_into_field_elements()
            .expect("invalid verifier message type")
            .remove(0);
        let r1 = transcript_messages.verifier_round((namespace, 1))[0]
            .clone()
            .try_into_field_elements()
            .expect("invalid verifier message type")
            .remove(0);
