...
fn create_subprotocol_namespace(current_namespace: &NameSpace, subprotocol_id: u32) -> NameSpace {
...
impl MessageBookkeeper {
    pub fn new_namespace(&mut self, namespace: &NameSpace) {
        ...
        self.map.insert(namespace.clone(), MessageIndices {
    ...
    pub fn fetch_node(&self, namespace: &NameSpace) -> Option<&MessageIndices> {
...
#[derive(Clone)]
pub struct Transcript<P: MTConfig, S: CryptographicSponge, F: PrimeField, L: LDT<P, F>> {
    ...
    P::InnerDigest: Absorb,
{
    ...
    pub fn new(sponge: S, hash_params: MTHashParameters<P>) -> Self {
        Self {
            ...
            bookkeeper: MessageBookkeeper{map: BTreeMap::new()},
            sponge,
            hash_params,
            _ldt: PhantomData
        }
    ...
    fn send_oracle_evaluations_unchecked(
        ...
        self.prover_message_oracles.push(ProverMessage::ReedSolomonCode {oracle: encoded, degree_bound, _mt_config: PhantomData});
    ...
    pub fn send_message_oracle(
        ...
        self.sponge.absorb(&mt_root);
        // store the encoded prover message for generating proof
        self.prover_message_oracles.push(ProverMessage::MessageOracle {oracle: encoded, _mt_config: PhantomData});
        self.attach_latest_prover_message_to_namespace(namespace);
    ...
    pub fn squeeze_verifier_field_elements(&mut self,
                                           namespace: &NameSpace,
                                           field_size: &[FieldElementSize]) -> Vec<F> {
        // squeeze message
        let msg = self.sponge.squeeze_field_elements_with_sizes(field_size);
        // store the verifier message for later decision phase
        self.verifier_messages.push(VerifierMessage::FieldElements(msg.clone()));
    ...
    pub fn squeeze_verifier_bytes(&mut self, namespace: &NameSpace, num_bytes: usize) -> Vec<u8> {
        ...
        // store the verifier message for later decision phase
        self.verifier_messages.push(VerifierMessage::Bytes(msg.clone()));
        self.attach_latest_verifier_message_to_namespace(namespace);
        msg
    }
    ...
    pub fn squeeze_verifier_bits(&mut self, namespace: &NameSpace, num_bits: usize) -> Vec<bool> {
        ...
        // store the verifier message for later decision phase
        self.verifier_messages.push(VerifierMessage::Bits(msg.clone()));
        self.attach_latest_verifier_message_to_namespace(namespace);
        msg
    }
    ...
    fn attach_latest_prover_message_to_namespace(&mut self, namespace: &NameSpace) {
        ...
        self.bookkeeper.fetch_node_mut(namespace)
            .expect("namespace not found")
            .prover_message_locations.push(index);
    }
    ...
    fn attach_latest_verifier_message_to_namespace(&mut self, namespace: &NameSpace) {
        ...
        self.bookkeeper.fetch_node_mut(namespace)
            .expect("namespace not found")
            .verifier_message_locations.push(index);
    }
