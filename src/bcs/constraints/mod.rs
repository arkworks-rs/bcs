...
use crate::bcs::message::{RoundOracle, ProverRoundMessageInfo};
...
pub trait RoundOracleVar<F: PrimeField, Oracle: RoundOracle<F>> {
    ...
    fn query_rs_code(&mut self, starting_index: UInt32<F>, stride: u32) -> Result<Vec<Vec<FpVar<F>>>, Error> {
        ...
        let position: Result<Vec<_>, _> =
        (0..(oracle_length / stride)).map(
            |x| UInt32::addmany(&[starting_index, UInt32::constant(stride * x)])
        ).collect();
        let position = position?;
        let num_rs_codes = self.num_reed_solomon_codes_oracles();
        let query_responses = self.query(&position)?;
        let result: Vec<_> = query_responses.into_iter().map(|mut resp|{
