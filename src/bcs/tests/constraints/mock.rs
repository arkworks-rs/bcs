...
use crate::bcs::transcript::{NameSpace, MessageBookkeeper};
...
impl<S: SpongeWithGadget<CF>, CF: PrimeField + Absorb> IOPVerifierWithGadget<S, CF> for MockTest1Verifier<CF> {
    ...
    (namespace: &NameSpace,
     ...
     _verifier_parameter: &Self::VerifierParameter)
        -> Result<(), SynthesisError> where MT::InnerDigest: Absorb, MTG::InnerDigest: AbsorbGadget<CF> {
        // prover send
    ...
    fn initial_state_for_query_and_decision_phase_var(public_input: &Self::PublicInputVar) -> Result<Self::VerifierStateVar, SynthesisError> {
        Ok(())
    }

    fn query_and_decide_var(cs: ConstraintSystemRef<CF>,
                            namespace: &NameSpace,
                            ...
                            bookkeeper: &MessageBookkeeper) -> Result<Self::VerifierOutputVar, SynthesisError> {
        ...
        prover_message_oracle[0].get_short_message(0).enforce_equal(pm1_1.as_slice())?;

        prover_message_oracle[0].query(&[UInt8::new_witness(cs.cs(), ||Ok(123))?.to_bits_le()?,
                UInt8::new_witness(cs.cs(), ||Ok(223))?.to_bits_le()?])?.into_iter()
            .zip(vec![vec![pm1_2[123].clone(), pm1_3[123].clone()],vec![pm1_2[223].clone(), pm1_3[223].clone()]].into_iter())
            .try_for_each(
             |(left, right)|left.enforce_equal(&right)
            )?;

        let vm1_1 = verifier_messages[0][0].clone().try_into_field_elements().unwrap();
